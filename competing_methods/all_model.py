...
def load_data(simu):
    if simu:
        ...
        for i in range(n_samples):
            ...
            n_il = len(times_il)
            Y_i = np.column_stack(
                (np.array([id[i]] * n_il), times_il, np.array(Y_i_).T))
            if i == 0:
                Y_ = Y_i
            else:
                Y_ = np.row_stack((Y_, Y_i))
        df2 = pd.DataFrame(data=Y_, columns=["id", "T_long"] + time_dep_feat)
        data = pd.merge(df2, df1, on="id")
    else:
        # load PBC Seq
        robjects.r.source(os.getcwd() + "/lights/competing_methods/load_PBC_Seq")
...
def all_model(n_runs = 1, simu=True):
    ...
    for i in range(n_runs):
        ...
        robjects.r.source(os.getcwd() + "/lights/competing_methods/CoxNet")
        ...
        start = time()
        robjects.r.source(os.getcwd() + "/lights/competing_methods/MJLCMM")
        ...
        start = time()
        robjects.r.source(os.getcwd() + "/lights/competing_methods/JMBayes")
